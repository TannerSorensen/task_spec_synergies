...
open(FH_OUT, "> $outfile.O")   or die "cannot open TV$outfile.O\n";
	...
	(print FH_OUT "$frame_dur $frame_last $phon_onset\n\n\n") or die "cannot write to $outfile: $!";
...
sub parse_ARPA_str {
	...
	$ons =~ s/N G/NX/g;			# restore velar digraphs (NG)
	...
	$cod =~ s/N G/NX/g;			# restore velar digraphs (NG)
