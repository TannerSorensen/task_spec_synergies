function U_vel = get_Uvel(contour_data, U_jaw, variant_switch)
...
if strcmp(variant_switch,'toutios2015factor')
    ...
    sections_id = contour_data.sections_id;
    sec_id_2 = [sections_id,sections_id];
    vt_section = 12; % velum label
    Dnorm_zero = Dnorm;
    Dnorm_zero(:,~ismember(sec_id_2,vt_section))=0;

    % principal components analysis of the velum, minus the jaw component
    [U_velraw,~,latent] = pca(Dnorm_zero);
    ...
    q_max = 3;
    U_vel = R*U_velraw(:,1:q_max)/chol(diag(latent(1:q_max)));
