...
#define MAX_FINITE_ITERATIONS 20
...
#define FNP_TOL	1.0e-5	/* Root tolerance (From Dave Williams code) */
...

{
  if(frame->an < AN_NO_NASAL_BREAKPOINT){
    ...
    *****/
    ...
    llframe->FNZ = llframe->FNP = (short) (speaker->fno + 0.5f);
    llframe->BNZ = llframe->BNP = (short) (speaker->NasalBandwidth + 0.5f);
  }
  else {
    ...
    llframe->FNZ = (short) (FNZ + 0.5f);
    llframe->BNZ = (short) (BNZ + 0.5f);
    llframe->FNP = (short) (FNP + 0.5f);
    llframe->BNP = (short) (BNP + 0.5f);
  }
}
...

{
  ...
  *****/
  ...
  fn = InterpolateTable(speaker->anfnTable,MAXANFN,an) *
    (speaker->fno / speaker->anfnTable_fno);
  ...
  *****/
  ...
  LOverA = InterpolateTable(speaker->f1LOverATable,MAXF1LOVERA,
    state->f1c);

  MmOverMn = (float)(LOverA * an / (3.7 * an + 100.));

  *pFNZ = fn * (float) sqrt((1. + MmOverMn) 
  ...
  *****/
  ...
  if(state->f1c < speaker->BNZ_f1BreakPoint)
    *pBNZ = speaker->NasalBandwidth;

  else
    *pBNZ = speaker->NasalBandwidth + 100.0f * MmOverMn;
...

{
  ...
  *****/
  ...
  if(frame->f1 >= speaker->fm_f1BreakPoint)
    fm = (float)(0.8 * frame->f2 + 0.2 * frame->f3);

  else{
    r = (float)(1.0 + 0.0143 * (frame->f1 - speaker->fm_f1BreakPoint));
    fm = (float)(r * (0.8 * frame->f2 + 0.2 * frame->f3) 
      + (1.-r) * 3000.);
  }
    ...
    *pb1x = temp;
    ...
    if(an <= speaker->BNP_B1_anLow)
      ...
      *pb1x = LinearInterpolate(an,
                speaker->BNP_B1_anLow,speaker->NasalBandwidth,
...

{
  ...
  *****/
  ...
  FNPvars.fn = InterpolateTable(speaker->anfnTable,MAXANFN,an) 
    * (speaker->fno / speaker->anfnTable_fno);
...

{
  ...
  *Brac_high = (float)(
    FNPvars->fn + (1.0-FINITE_OFFSET) * (FNPvars->fp - FNPvars->fn)
    );
  ...
  for( i = 0; i < MAX_FINITE_ITERATIONS; i++ )
    if( SusceptanceSum(*Brac_low,(void*)FNPvars) <= 0.0 )
      break;
    else
      *Brac_low -= (float)((1.0-FINITE_OFFSET) * (*Brac_low - min_f));
  ...
  for( i = 0; i < MAX_FINITE_ITERATIONS; i++ )
    if( SusceptanceSum(*Brac_high,(void*)FNPvars) >= 0.0 )
      break;
    else
      *Brac_high += (float)((1.0-FINITE_OFFSET) * (max_f - *Brac_high));
