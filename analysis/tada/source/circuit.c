...
#define CIRCUIT_TOL 2.0e-3 /* Root tolerance */
...

{
  ...
  float MinGuess,MaxGuess,Psm_cgs = CMWATER_TO_CGS(speaker->Psm);
  ...
  *****/
  ...
  NumInterpolations = round(speaker->UpdateInterval
			    / speaker->NewtonInterpTimeStep);

  agprev = MMSQ_TO_CMSQ(oldframe->ag);
  agstep = (MMSQ_TO_CMSQ(frame->ag) - agprev) / (float) NumInterpolations;

  acxprev = MMSQ_TO_CMSQ(oldstate->acx);
  acxstep = (MMSQ_TO_CMSQ(state->acx) - acxprev) / (float) NumInterpolations;

  anprev = MAX(0.0f, MMSQ_TO_CMSQ(oldframe->an));
  anstep = (MAX(0.0f, MMSQ_TO_CMSQ(frame->an)) - anprev) / (float) NumInterpolations;

  ueprev = oldframe->ue;
  uestep = (frame->ue - ueprev) / (float) NumInterpolations;
  
  apprev = MMSQ_TO_CMSQ(oldframe->ap);
  apstep = (MMSQ_TO_CMSQ(frame->ap) - apprev) / (float) NumInterpolations;

  psprev = CMWATER_TO_CGS(oldframe->ps);
  psstep = (CMWATER_TO_CGS(frame->ps) - psprev) / (float) NumInterpolations;

  state->Cg = speaker->Cgm + speaker->KCg * 0.01f * frame->dc * speaker->Cgm;
  Cgprev = oldstate->Cg;
  Cgstep = (state->Cg - Cgprev) / (float) NumInterpolations;

  state->Cw = speaker->Cwm + speaker->KCw * 0.01f * frame->dc * speaker->Cwm;
  Cwprev = oldstate->Cw;
  Cwstep = (state->Cw - Cwprev) / (float) NumInterpolations;
  ...
  *****/
  ...
  deltaTOver2 = 0.5f * speaker->UpdateInterval/NumInterpolations;
  ...
  *****/
  ...
  otherArgs.rootTwoOverRho = (float)sqrt(2.0 / RHO);
  ...
  *****/
  state->Ug = (float)(
    FLOW_SQRT(2.0 * (CMWATER_TO_CGS(frame->ps) - state->Pm) / RHO)
    * MMSQ_TO_CMSQ(state->agf)
    );
  
  state->Uacx = (float)(FLOW_SQRT(2.0 * state->Pm / RHO) * MMSQ_TO_CMSQ(state->acx));
  state->Un = (float)(FLOW_SQRT(2.0 * state->Pm / RHO) * MAX(0.0f, MMSQ_TO_CMSQ(frame->an)));
