...
=head1 REVISION
 ...
 Last Updated:	21-Nov-07	Changed processing of <onsets> to include cluster-specific gestural rules
...
open(FH_DICT, "<:utf8", "$pdict") or die "cannot open $pdict\n";
	my $cnt = 0;
	while (<FH_DICT>) {
		if (/^($lc)\s+([A-Z0-4_\-\(\) ]+)/) {
			#print "\n  $1\t$2";
...
print "  parsing input string    <$orthog> ...  ";
	@words	= split( /#+/, $orthog);
	$no_wds	= @words;
	print " found $no_wds word(s)\n\n";

	#-------------------------------------------------------------------------
	open(FH_TST, ">:utf8", "test.txt") or die "cannot open test.txt\n";
		foreach $wd_orth (@words) {
			(print FH_TST "$wd_orth \n") or die "cannot write to test.txt: $!";
		}
	close (FH_TST);
	#-------------------------------------------------------------------------
...
open(FH_OUT, "> $outfile.O")   or die "cannot open TV$outfile.O\n";
	...
	my	$syll_cnt = 1;										# syllables counted over entire phrase
	foreach $wd_orth (@words) {
		...
		(print FH_OUT "%\n") or die "cannot write to $outfile: $!";
		(print FH_OUT "% Word $wd_cnt:\t$wd_orth\n") or die "cannot write to $outfile: $!";
						...
						if ( $arpabet{$1} ) {
							...
							if ($onc eq "ons") {
								foreach $rule (values %ons_rules) {
									if ( $rule->{Ct} eq $seg ) {
										if ( ( ($rule->{C1} eq $last_seg) && ($rule->{C2} eq $seg) ) || ( ($rule->{C2} eq $next_seg) && ($rule->{C1} eq $seg) ) || ( ($rule->{C2} eq "*") && ($rule->{C1} eq $seg) ) ) {
											if ( $rule->{TARG} ) {
