...
The study finds that the jaw contributes more to the coronal stop in \textipa{[t]} than to bilabial stop, palatal approximation, velar stop, and pharyngeal approximations in \textipa{[p]}, \textipa{[i]}, \textipa{[k]}, and \textipa{[A]}, 
%that the jaw contributes more to the palatal stop constriction task in \textipa{[i]} than to 
%
...
(coronal stop inter-quartile range: \SIrange{\Sexpr{signif(100*quantile((with(tab,bm[tv==2])),0.25),2)}}{\Sexpr{signif(100*quantile((with(tab,bm[tv==2])),0.75),2)}}{\percent};
pharyngeal approximation inter-quartile range: \SIrange{\Sexpr{signif(100*quantile((with(tab,bm[tv==5])),0.25),2)}}{\Sexpr{signif(100*quantile((with(tab,bm[tv==5])),0.75),2)}}{\percent}),
...
$p=\Sexpr{signif(stat_test$test$pvalues[5],2)}$),
%
...
$p=\Sexpr{signif(stat_test$test$pvalues[7],2)}$).
%
%
%
...
$p=\Sexpr{signif(stat_test$test$pvalues[10],2)}$).
...
%
bilabial stop in \Sexpr{sum(p[,1] < 0.05)}/\Sexpr{nrow(p)} cases 
(\SI{\Sexpr{signif(100*sum(p[,1] < 0.05)/nrow(p),2)}}{\percent}), 
%
palatal approximation in \Sexpr{sum(p[,5] < 0.05)}/\Sexpr{nrow(p)} cases
(\SI{\Sexpr{signif(100*sum(p[,5] < 0.05)/nrow(p),2)}}{\percent}), 
velar stop in \Sexpr{sum(p[,6] < 0.05)}/\Sexpr{nrow(p)} cases 
(\SI{\Sexpr{signif(100*sum(p[,6] < 0.05)/nrow(p),2)}}{\percent}), 
and pharyngeal approximation in \Sexpr{sum(p[,7] < 0.05)}/\Sexpr{nrow(p)} cases
...
%
velar stop in \Sexpr{sum(p[,8] < 0.05)}/\Sexpr{nrow(p)} cases
(\SI{\Sexpr{signif(100*sum(p[,8] < 0.05)/nrow(p),2)}}{\percent})
and pharyngeal approximation in \Sexpr{sum(p[,9] < 0.05)}/\Sexpr{nrow(p)} cases
...
%
velar stop in \Sexpr{sum(p[,10] < 0.05)}/\Sexpr{nrow(p)} cases 
...
%
The likelihood ratio test indicates that the by-participant random slopes for constriction task are a significant source of variance ($\chi^2(\Sexpr{a0$`Chi Df`[2]})= \Sexpr{signif(a0$Chisq[2],2)}; p = \Sexpr{signif(a0$`Pr(>Chisq)`[2],2)}$).
%
...
The results of the Mann-Whitney Tests demonstrate that the pattern of average values across constriction tasks is consistent with the effect for constriction task discovered by the linear mixed effects model (cf. Section~\ref{sec:taskspec}; coronal stop $>$ palatal approximation $>$ \{velar stop, pharyngeal approximation\} and coronal stop $>$ bilabial stop) with only three exceptions: velar stop-pharyngeal approximation constrast of participant m1; palatal approximation-velar stop and palatal approximation-pharyngeal approximation constrasts of m4.
...
For this reason, the Mann-Whitney U Test fails to reject every null hypotheses the linear mixed effects model analysis rejects using pooled data from all participants.
...
The present study demonstrates that speakers use the jaw more for the coronal stop than for the bilabial stop, palatal approximation, velar stop, and pharyngeal approximation,
%
that speakers use the jaw more for the palatal approximation than for the velar stop and pharyngeal approximation,
%
and that speakers use the jaw more for the velar stop than for the pharyngeal approximation
%
...
% TODO: double check
The present study demonstrates that speakers use the jaw more for the coronal stop than for the velar stop and pharyngeal approximation.
%
The pattern of jaw, tongue, and lip usage is heterogeneous for bilabial stop and palatal approximation.
%
