...
%
% TODO: is this finding right?
The study finds that the jaw contributes more to the coronal gesture for \textipa{[t]} than to bilabial, velar, and pharyngeal gestures for \textipa{[p]} \textipa{[k]}, and \textipa{[A]}.
%
Differences in inter-articulator coordination by constriction task support the claim that articulator synergies are task-dependent.
\end{abstract}
...
%
Our previous study on unperturbed speech suggests that healthy adult speakers of American English may use the jaw more for constrictions at the bilabial, coronal, and palatal places of articulation than for posterior constrictions at the velar and pharyngeal places of articulation~\citep{Sorensen+2016}. 
%
...
Section~\ref{sec:patterns} investigates differences by participant in the effect of constriction task on the articulator synergy biomarker.
...
\caption{(color online) Quantitative readout of the contributions of the jaw (dark bar) and tongue (light bar) to a palatal constriction for \textipa{[i]} during the transition from \textipa{[A]} to \textipa{[i]} and back to \textipa{[A]} in the sequence \textipa{[AiA]}. 
%
...
\caption{(color online) Comparison of intraclass correlation coefficients (ICC) for different numbers of jaw factors (color) and for different numbers of tongue and lip factors ($x$-axis) for the bilabial constriction task of \textipa{[ApA]}, coronal constriction task of \textipa{[AtA]}, palatal constriction task of \textipa{[AiA]}, velar constriction task of \textipa{[AkA]}, and pharyngeal constriction task of the second \textipa{[A]} in \textipa{[AiA]}.}
\label{fig:icc_all}
...
<<repeatability,echo=FALSE,results='hide',message=FALSE,warning=FALSE>>=
...
# GET MIN, MEDIAN, AND MAX ICC AT EACH PLACE
############################################

minICCs <- array(NA,dim=c(5))
maxICCs <- array(NA,dim=c(5))
medICCs <- array(NA,dim=c(5))

tv_loc_idx <- bilabial_idx
...
minICCs[tv_loc_idx] <- min(icc)
medICCs[tv_loc_idx] <- median(icc)
maxICCs[tv_loc_idx] <- max(icc)

for(tv_loc_idx in seq(coronal_idx,pharyngeal_idx)){
...
% bilabial place
%
The bilabial constriction task for \textipa{[p]} had
\Sexpr{
ifelse(getICCrating(minICCs[bilabial_idx])==getICCrating(maxICCs[bilabial_idx]),
getICCrating(maxICCs[bilabial_idx]),
paste(getICCrating(minICCs[bilabial_idx]),
" to ",
getICCrating(maxICCs[bilabial_idx])))
}
repeatability
(range: \numrange{\Sexpr{signif(minICCs[bilabial_idx],2)}}{\Sexpr{signif(maxICCs[bilabial_idx],2)}},
median: \num{\Sexpr{signif(medICCs[bilabial_idx],2)}}). 
%
% coronal place
% 
The coronal constriction task for \textipa{[t]} had
\Sexpr{
ifelse(getICCrating(minICCs[coronal_idx])==getICCrating(maxICCs[coronal_idx]),
getICCrating(maxICCs[coronal_idx]),
paste(getICCrating(minICCs[coronal_idx]),
" to ",
getICCrating(maxICCs[coronal_idx])))
}
repeatability
(range: \numrange{\Sexpr{signif(minICCs[coronal_idx],2)}}{\Sexpr{signif(maxICCs[coronal_idx],2)}},
median: \num{\Sexpr{signif(medICCs[coronal_idx],2)}}). 
%
% palatal place
%
The palatal constriction task for \textipa{[i]} had
\Sexpr{
ifelse(getICCrating(minICCs[palatal_idx])==getICCrating(maxICCs[palatal_idx]),
getICCrating(maxICCs[palatal_idx]),
paste(getICCrating(minICCs[palatal_idx]),
" to ",
getICCrating(maxICCs[palatal_idx])))
}
repeatability
(range: \numrange{\Sexpr{signif(minICCs[palatal_idx],2)}}{\Sexpr{signif(maxICCs[palatal_idx],2)}},
median: \num{\Sexpr{signif(medICCs[palatal_idx],2)}}). 
%
% velar place
%
The velar constriction task for \textipa{[k]} had
\Sexpr{
ifelse(getICCrating(minICCs[velar_idx])==getICCrating(maxICCs[velar_idx]),
getICCrating(maxICCs[velar_idx]),
paste(getICCrating(minICCs[velar_idx]),
" to ",
getICCrating(maxICCs[velar_idx])))
}
repeatability
(range: \numrange{\Sexpr{signif(minICCs[velar_idx],2)}}{\Sexpr{signif(maxICCs[velar_idx],2)}},
median: \num{\Sexpr{signif(medICCs[velar_idx],2)}}). 
%
% pharyngeal place
%
The pharyngeal constriction task for \textipa{[A]} had 
\Sexpr{
ifelse(getICCrating(minICCs[pharyngeal_idx])==getICCrating(maxICCs[pharyngeal_idx]),
getICCrating(maxICCs[pharyngeal_idx]),
paste(getICCrating(minICCs[pharyngeal_idx]),
" to ",
getICCrating(maxICCs[pharyngeal_idx])))
}
repeatability
(range: \numrange{\Sexpr{signif(minICCs[pharyngeal_idx],2)}}{\Sexpr{signif(maxICCs[pharyngeal_idx],2)}},
median: \num{\Sexpr{signif(medICCs[pharyngeal_idx],2)}}). 
...
if(morphology_dataset==TRUE){
  ...
  tab <- subset(tab, n_jaw==1 & n_tng==6 & n_lip==2)
}
...
The range of ICC values obtained for the coronal, velar, and pharyngeal constriction tasks included some ICC values in the poor to weak range. 
%
The reason for this may be different for the different constriction tasks. 
%
For the velar constriction task, the total variance of the biomarker is small
(inter-quartile range: \SIrange{\Sexpr{signif(100*quantile((with(tab,bm[tv==4])),0.25),2)}}{\Sexpr{signif(100*quantile((with(tab,bm[tv==4])),0.75),2)}}{\percent}). 
...
%
In contrast, for the coronal and pharyngeal constriction tasks, the total variance of the biomarker is large 
(coronal inter-quartile range: \SIrange{\Sexpr{signif(100*quantile((with(tab,bm[tv==2])),0.25),2)}}{\Sexpr{signif(100*quantile((with(tab,bm[tv==2])),0.75),2)}}{\percent};
pharyngeal inter-quartile range: \SIrange{\Sexpr{signif(100*quantile((with(tab,bm[tv==5])),0.25),2)}}{\Sexpr{signif(100*quantile((with(tab,bm[tv==5])),0.75),2)}}{\percent}),
suggesting that intra-participant variance is substantial (see the histograms in Fig.~\ref{fig:histograms}). 

Although the velar and pharyngeal constriction tasks may not involve a very great contribution of the jaw, the number of jaw factors nevertheless affects ICC (see panels ``velar'' and ``pharyngeal'' of Fig.~\ref{fig:icc_all}). This is due to the fact that using more jaw factors increases the variance of the tongue that is explained by jaw factors (cf. Fig.~\ref{fig:varexpl}, panel b), and some of this variance may reflect the performance of velar and pharyngeal constrictions. 
...
Sample distribution of the articulator synergy biomarker for the bilabial, coronal, palatal, velar, and pharyngeal constriction tasks. The biomarker indicates the percent of a constriction that was produced by the jaw.
...
The coronal constriction had
%
% note: minus sign is due to the direction of the contrast (i.e., tv1 - tv2 rather than tv2 - tv1)
\SI{\Sexpr{signif(-100*stat_test$test$coefficients["tv1 - tv2"],2)}}{\percent} 
larger biomarker values than the bilabial constriction
($z=\Sexpr{signif(-stat_test$test$tstat["tv1 - tv2"],2)}$, 
$p=\Sexpr{stat_test$test$pvalues[1]}$),
%
\SI{\Sexpr{signif(100*stat_test$test$coefficients["tv2 - tv3"],2)}}{\percent} 
larger biomarker values than the palatal constriction
($z=\Sexpr{signif(stat_test$test$tstat["tv2 - tv3"],2)}$, 
$p=\Sexpr{stat_test$test$pvalues[5]}$),
%
\SI{\Sexpr{signif(100*stat_test$test$coefficients["tv2"],2)}}{\percent} 
larger biomarker values than the velar constriction
($z=\Sexpr{signif(stat_test$test$tstat["tv2"],2)}$, 
$p=\Sexpr{stat_test$test$pvalues[6]}$),
%
and \SI{\Sexpr{signif(100*stat_test$test$coefficients["tv2 - tv5"],2)}}{\percent} 
larger biomarker values than the pharyngeal constriction
($z=\Sexpr{signif(stat_test$test$tstat["tv2 - tv5"],2)}$, 
$p=\Sexpr{stat_test$test$pvalues[7]}$).
%
%
%
...
%
\SI{\Sexpr{signif(100*stat_test$test$coefficients["tv2 - tv3"],2)}}{\percent} 
smaller biomarker values than the coronal constriction (see immediately above),
%
the palatal constriction had
%
\SI{\Sexpr{signif(100*stat_test$test$coefficients["tv3"],2)}}{\percent} 
larger biomarker values than the velar constriction
($z=\Sexpr{signif(stat_test$test$tstat["tv3"],2)}$, 
$p=\Sexpr{stat_test$test$pvalues[8]}$)
%
and \SI{\Sexpr{signif(100*stat_test$test$coefficients["tv3 - tv5"],2)}}{\percent} 
larger biomarker values than the pharyngeal constriction
($z=\Sexpr{signif(stat_test$test$tstat["tv3 - tv5"],2)}$, 
$p=\Sexpr{stat_test$test$pvalues[9]}$).
%
%
%
The velar constriction had 
%
\SI{\Sexpr{signif(100*stat_test$test$coefficients["tv5"],2)}}{\percent} 
smaller biomarker values than the pharyngeal constriction
($z=\Sexpr{signif(stat_test$test$tstat["tv5"],2)}$, 
$p=\Sexpr{stat_test$test$pvalues[10]}$).

We infer that the jaw contributed significantly more to the coronal constriction than to the bilabial, palatal, velar, and pharyngeal constrictions,
%
that the jaw contributed significantly more to the palatal constriction than to the velar and pharyngeal constrictions,
%
and that the jaw contributed significantly less to the pharyngeal constriction than to the velar constriction.
%
...
The sample distribution of the articulator synergy biomarker for the velar constriction task had small dispersion about a distinct peak at \SI{10}{\percent} 
%
...
%
The sample distribution of the articulator synergy biomarker for the coronal constriction task had a distinct peak at \SI{60}{\percent}
%
...
%
The distinctly peaked sample distributions of the articulator synergy biomarkers for the coronal and velar constriction tasks likely contributed to the statistically significant bilabial-velar, coronal-palatal, coronal-velar, palatal-velar, palatal-pharyngeal, velar-pharyngeal, and coronal-pharyngeal contrasts.

<<<<setup, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE>>=
...
source(file.path(".","require_libraries.R"))

# LOAD USER-DEFINED FUNCTIONS
#############################

source(file.path(".","declare_user_defined_functions.R"))
...
z <- matrix(nrow = n, ncol = 10)
p <- matrix(nrow = n, ncol = 10)
...
%
The articulator synergy biomarker was greater for the coronal constriction than for the 
%
bilabial constriction in \Sexpr{sum(p[,1] < 0.05)}/\Sexpr{nrow(p)} cases 
(\SI{\Sexpr{signif(100*sum(p[,1] < 0.05)/nrow(p),2)}}{\percent}), 
%
palatal constriction in \Sexpr{sum(p[,5] < 0.05)}/\Sexpr{nrow(p)} cases
(\SI{\Sexpr{signif(100*sum(p[,5] < 0.05)/nrow(p),2)}}{\percent}), 
velar constriction in \Sexpr{sum(p[,6] < 0.05)}/\Sexpr{nrow(p)} cases 
(\SI{\Sexpr{signif(100*sum(p[,6] < 0.05)/nrow(p),2)}}{\percent}), 
and constriction pharyngeal in \Sexpr{sum(p[,7] < 0.05)}/\Sexpr{nrow(p)} cases
(\SI{\Sexpr{signif(100*sum(p[,7] < 0.05)/nrow(p),2)}}{\percent}).
%
The articulator synergy biomarker was greater for the palatal constriction than for the 
%
velar constriction in \Sexpr{sum(p[,8] < 0.05)}/\Sexpr{nrow(p)} cases
(\SI{\Sexpr{signif(100*sum(p[,8] < 0.05)/nrow(p),2)}}{\percent})
and pharyngeal constriction in \Sexpr{sum(p[,9] < 0.05)}/\Sexpr{nrow(p)} cases
(\SI{\Sexpr{signif(100*sum(p[,9] < 0.05)/nrow(p),2)}}{\percent}).
%
The articulator synergy biomarker was greater for the pharyngeal constriction than for the 
%
velar constriction in \Sexpr{sum(p[,10] < 0.05)}/\Sexpr{nrow(p)} cases 
(\SI{\Sexpr{signif(100*sum(p[,10] < 0.05)/nrow(p),2)}}{\percent}).
%
Overall, these results support the inference that the jaw contributed significantly more to the coronal constriction task than to the bilabial and velar constriction tasks,
%
that the jaw contributed significantly more to the palatal constriction task than to the velar constriction task.
%
However, the coronal-palatal, coronal-pharyngeal, palatal-pharyngeal, and velar-pharyngeal contrasts should be interpreted with caution, since the effect size is smaller than for the more robust effects and the significance of these contrasts depends on parameterization.
...
<<patterns,echo=FALSE,results='hide',message=FALSE,warning=FALSE>>=

# PERFORM STATISTICAL TEST
##########################

m_velar <- lmer(bm ~ 1 + tv + (1 + tv | participant), tab, REML=FALSE)
m0 <- lmer(bm ~ 1 + tv + (1 | participant), tab, REML=FALSE)
a0 <- anova(m_velar,m0)
@
...
%
This section investigates between- and within-participant variability in this effect.
%
Between-participant variability is evaluated by testing the significance of by-participant random slopes for constriction task.
%
...
%
The likelihood ratio test indicates that the by-participant random slopes for constriction task are a significant source of variance ($\chi^2(\Sexpr{a0$`Chi Df`[2]})= \Sexpr{a0$Chisq[2]}; p = \Sexpr{a0$`Pr(>Chisq)`[2]}$).
%
This indicates by-participant variability in the effect for constriction task.
%
By-participant variability in the effect for constriction task is evaluated by determining whether individual participants display similar effects for constriction task.
%
...
%
For six of eight participants, the coronal constriction task has larger biomarker values than the bilabial, palatal, velar, and pharyngeal constriction tasks.
These participants display effects that are consistent with the significant coronal-velar and coronal-pharyngeal contrasts identified with the linear mixed effects model of Section~\ref{sec:taskspec}, but also support 
Additionally, this pattern suggests that the coronal constriction task has larger biomarker values than 

Do participants have high variance for some constriction tasks but low variance at others, 
or does a given participant have constant variance across constriction tasks?
%
The present study applies the Fligner-Killeen Test to determine whether different constriction tasks have different biomarker variances.
%
TODO: details of test The test is applied to the data of each participant separately.
%
The Fligner-Killeen Test is applied to pairs of constriction tasks to test for differences in variance for each participant separately. 

The study performed $(\text{number of participants}) \times ((\text{number of constriction tasks})^2 - (\text{number of constriction tasks}))/2 = 80$ Mann-Whitney U Tests and the same number of Fligner-Killeen tests for a total of \num{160} statistical tests. 
%
...
The present study demonstrates that speakers use the jaw more for the coronal constriction task than for the velar and pharyngeal constriction tasks. 
%
The pattern of jaw, tongue, and lip usage is heterogeneous for the bilabial, coronal, and palatal constriction tasks.
%
...
%
The articulator synergy biomarker does not have systematic bias. However, precision is weak or poor at the coronal, velar, and pharyngeal constriction tasks. 
%
...
%
For the velar constriction task, low precision is due to small inter-participant variance (cf. Fig.~\ref{fig:histograms}). 
%
For the coronal and pharyngeal constriction tasks, low precision is due to large intra-participant variance.
%
