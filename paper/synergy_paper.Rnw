...
%
The finding that the articulator synergy biomarker was greater at the coronal place than at the velar place was reproduced with \Sexpr{sum(p[,2] < 0.05)} of \Sexpr{nrow(p)} parameterizations (\SI{\Sexpr{signif(100*sum(p[,2] < 0.05)/nrow(p),2)}}{\percent} of cases).
%
The finding that the articulator synergy biomarker was greater at the coronal place than at the pharyngeal place was reproduced with \Sexpr{sum(p[,5] < 0.05)} of \Sexpr{nrow(p)} parameterizations (\SI{\Sexpr{signif(100*sum(p[,5] < 0.05)/nrow(p),2)}}{\percent} of cases).
%
...
Section~\ref{sec:taskspec} shows an effect of place of articulation on the articulator synergy biomarker values. 
%
This section investigates between- and within-participant variability in this effect.
%
Between-participant variability is evaluated by testing the significance of by-participant random slopes for place of articulation.
%
The linear mixed effects model of Section~\ref{sec:taskspec} (cf. Equation~\ref{eq:lmm}) is compared to a reduced model that does not have by-participant random slopes for place of articulation.
%
The likelihood ratio test indicates that the by-participant random slopes for place of articulation are a significant source of variance ($\chi^2(\Sexpr{a0$`Chi Df`[2]})= \Sexpr{a0$Chisq[2]}; p = \Sexpr{a0$`Pr(>Chisq)`[2]}$).
%
This indicates by-participant variability in the effect for place of articulation.
%
By-participant variability in the effect for place of articulation is evaluated by determining whether individual participants display similar effects for place of articulation.
%
For each participant, the Mann-Whitney U Test identifies all the places of articulation that differ in average biomarker value.
%
For six of eight participants, the coronal place of articulation has larger biomarker values than the bilabial, palatal, velar, and pharyngeal places of articulation.
...
Additionally, this pattern suggests that the coronal place of articulation has larger biomarker values than 
