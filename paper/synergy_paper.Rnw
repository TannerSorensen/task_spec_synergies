...
This section evaluates the bias of the articulator synergy biomarker. Bias is the difference between the expected value of a measurement and its true value. The true value of the articulator synergy biomarker is unknown for any given MRI scan of a particular subject. For this reason, the present study designed a digital phantom for which the true value of the biomarker could be controlled. By varying the biomarker over the full range of possible values,

The vowel-consonant-vowel sequences consisted of two subsequent gestures: [apa] consisted of bilabial closure followed by release; [ata] consisted of alveolar closure followed by release; [aja] consisted of palatal approximation followed by pharyngeal approximation; [aka] consisted of velar closure followed by release.

Following Task Dynamics~\citep{saltzman1989dynamical}, change in the vector $\mathbf{z}$ of constriction degrees evolves according to the following equation for time $t_a \leq t \leq t_b$.
\begin{equation}
\label{eq:td_basic}
\mathbf{\ddot{z}} = -\mathbf{K}(t)\left[\mathbf{z} - \mathbf{z}_0(t)\right] -\mathbf{B}(t)\mathbf{\dot{z}}
\end{equation}
where $\mathbf{z}_0$ is a vector of $m$ constriction degree targets and $\mathbf{K}$ and $\mathbf{B}$ are $m \times m$ diagonal matrices of stiffness and damping coefficients, respectively.
The matrices $\mathbf{K}$ and $\mathbf{B}$ are constant for a given gesture.
When the constriction is for constriction degree $z_i$, then stiffness $k_{ii} = \omega^2$, damping $b_{ii} = 2\omega$, and $k_{jj} = b_{jj} = 0$ for $j \neq i$, where $\omega$ is the natural frequency of the gesture.
The vector $\mathbf{z}_0$ of constriction degree targets depended on the gesture: $z_{01} = $~\SI{0}{\milli\meter} for bilabial closure; $z_{01} = $~\SI{7.1}{\milli\meter} for bilabial release; $z_{02} = $~\SI{0}{\milli\meter} for alveolar closure; $z_{02} = $~\SI{7.1}{\milli\meter} for alveolar release; $z_{03} = $~\SI{4.8}{\milli\meter} for palatal approximation; $z_{04} = $~\SI{0}{\milli\meter} for velar closure; $z_{04} = $~\SI{7.1}{\milli\meter} for velar release; and $z_{05} = $~\SI{4.8}{\milli\meter} for pharyngeal approximation.
The natural frequency $\omega = 10$ and simulation duration (i.e. start time $t_a=0$ and end time $t_b=1.6$) were set arbitrarily, as the relative usage of the jaw, tongue, and lips does not depend on the time-scale of the simulation.

Given model parameters $\mathbf{K}$, $\mathbf{B}$, $\mathbf{z}_0$ and initial conditions $z(t_a)$, $\dot{z}(t_a)$, the solution $\mathbf{z}(t)$ to Equation~\ref{eq:td_basic} is unique for $t_a \leq t \leq t_b$. The unique solution $\mathbf{z}(t)$ maps to many paths $\mathbf{w}(t)$ of factor scores. One particular path $\mathbf{w}(t)$ is determined from the set of many possible paths by the forward kinematic map $\mathbf{G}$, its jacobian $\mathbf{J}(\mathbf{w})$, and a set of weights $v_1, v_2, \ldots, v_q$ for the $q$ factors.
The unique path $\mathbf{w}(t)$ of factor scores is the solution to the following equation over the time interval $t_a \leq t \leq t_b$.
\begin{equation}
...
\end{equation}
This follows from the change of variables $\mathbf{z} = \mathbf{G}(\mathbf{w})$, $\mathbf{\dot{z}} = \mathbf{J}(\mathbf{w})\mathbf{\dot{w}}$, $\mathbf{\ddot{z}} = \mathbf{J}(\mathbf{w})\mathbf{\ddot{w}} + \mathbf{\dot{J}}(\mathbf{w},\mathbf{\dot{w}})\mathbf{\dot{w}}$ and the weighted jacobian pseudoinverse $\mathbf{J}^+ = \mathbf{V}^{-1}\mathbf{J}^\intercal (\mathbf{J}\mathbf{V}^{-1}\mathbf{J}^\intercal)^{-1}$.

% TODO: explain the simulation parameters (how we solved the equation)

The dynamical system simulations generated paths $\mathbf{w}[n]$ of factor scores corresponding to the utterances \textipa{[apa]}, \textipa{[ata]}, \textipa{[aja]}, and \textipa{[aka]}. By performing the simulation for \SI{15} different values of the jaw weight $v_{11}$ ranging from \SI{0.01} to \SI{10000} and computing the biomarker value  for each utterance using Equations~\ref{eq:lambda} and~\ref{eq:nu}, the simulation generated synthetic data for which the true articulator synergy biomarker values are known. Figure~\ref{fig:validation} demonstrates that the jaw weight is inversely related to biomarker value: when the jaw weight is large, the biomarker value is close to \SI{0}{\percent} indicating a small jaw contribution; when the jaw weight is small, the biomarker value is close to \SI{100}{\percent}, indicating a large jaw contribution. 
...
for(tv_loc_idx in seq(alveolar_idx,pharyngeal_idx)){
...
\Sexpr{
ifelse(getICCrating(minICCs[alveolar_idx])==getICCrating(maxICCs[alveolar_idx]),
getICCrating(maxICCs[alveolar_idx]),
paste(getICCrating(minICCs[alveolar_idx]),
" to ",
getICCrating(maxICCs[alveolar_idx])))
}
repeatability
(range: \numrange{\Sexpr{signif(minICCs[alveolar_idx],2)}}{\Sexpr{signif(maxICCs[alveolar_idx],2)}},
median: \num{\Sexpr{signif(medICCs[alveolar_idx],2)}}). 
%
% palatal place
%
...
(alveolar inter-quartile range: \SIrange{\Sexpr{signif(100*quantile((with(tab,bm[tv==2])),0.25),2)}}{\Sexpr{signif(100*quantile((with(tab,bm[tv==2])),0.75),2)}}{\percent};
...
%
The finding that the articulator synergy biomarker was greater at the alveolar place than at the velar place was reproduced with \Sexpr{sum(p[,2] < 0.05)} of \Sexpr{nrow(p)} parameterizations (\SI{\Sexpr{signif(100*sum(p[,2] < 0.05)/nrow(p),2)}}{\percent} of cases).
%
The finding that the articulator synergy biomarker was greater at the alveolar place than at the pharyngeal place was reproduced with \Sexpr{sum(p[,5] < 0.05)} of \Sexpr{nrow(p)} parameterizations (\SI{\Sexpr{signif(100*sum(p[,5] < 0.05)/nrow(p),2)}}{\percent} of cases).
%
...
%
When using only 1 or 2 jaw factors, this number increases to \Sexpr{sum(p[jaw<3,5] < 0.05)} of \Sexpr{nrow(p[jaw<3,])} parameterizations (\SI{\Sexpr{signif(100*sum(p[jaw<3,5] < 0.05)/nrow(p[jaw<3,]),2)}}{\percent} of cases).
%
