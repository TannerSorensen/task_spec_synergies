...
for(f_val in seq(from = 20, to = 90, by = 10)){
  ...
  pdf(file.path(graphics_path,paste(paste("icc",gsub(" ","_",tv_loc[tv_loc_idx]),sep="_"),"_f",f_val,".pdf",sep="")))
  ...
  matplot(lip_fac,icc, 
          ...
          ylim=c(0,1), ylab="ICC", xlim=(range(lip_fac)+c(-0.5,0.5)), xlab=xlb, cex.lab=font_scale)
  axis(side=1, at=lip_fac, labels=xtlbs, col.ticks=0, col=NA, cex.axis=font_scale)
  ...
  for(tv_loc_idx in seq(2,5)){
    ...
    matplot(tng_fac,icc, 
            col=cols, type="b", pch=1, lty=1, lwd=2, axes=FALSE, las=1, cex=4,
            ylim=c(0,1), ylab="", xlim=(range(tng_fac)+c(-0.5,0.5)), xlab=xlb, cex.lab=font_scale)
    axis(side=1, at=tng_fac, labels=xtlbs, 
